...
@section('body')
  <div class="wrapper">
    ...
    @endif
      ...
      <div class="d-flex justify-content-between align-items-center">
        ...
        <form action="{{ route('manage-ukm.index') }}" method="GET" class="d-flex gap-2">
      ...
      <tr class="border">
        ...
        data-bs-target="#updateUkm-{{ $ukm->ukm_id }}"><i
        class="fa-regular fa-pen-to-square text-warning"></i></button>
        <button type="button" class="btn shadow-none" data-bs-toggle="modal"
        data-bs-target="#deleteUkm-{{ $ukm->ukm_id }}"><i
  ...
  @endforelse
      ...
      <div class="d-flex justify-content-center">
        ...
        <ul class="pagination">
          {{ $ukms->appends(['search' => request('search')])->onEachSide(1)->links('pagination::bootstrap-5') }}
        </ul>
        </nav>
      </div>
      </div>
  ...
  <div class="modal fade" id="addUkm" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <form action="{{ route('manage-ukm.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <input type="file" class="form-control shadow-none" id="profile_photo_ukm" name="profile_photo_ukm"
          aria-describedby="emailHelp">
        </div>
        <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">Nama UKM</label>
        <input type="text" class="form-control shadow-none" name="name_ukm" id="exampleInputEmail1"
          aria-describedby="emailHelp">
        </div>
        <div class="mb-3">
        <label for="exampleInputEmail1" class="form-label">Deskripsi UKM</label>
        <textarea type="text" class="form-control shadow-none" name="description" id="exampleInputEmail1"
  ...
  {{-- update ukm --}}
  @foreach ($ukms as $ukm)
    <div class="modal fade" id="updateUkm-{{ $ukm->ukm_id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <img src="{{ asset('storage/profile_photo_ukm/' . $ukm->profile_photo_ukm) }}" alt="Logo UKM"
        class="img-thumbnail mb-2" style="max-width: 100px;">
      <input type="file" class="form-control shadow-none" id="profile_photo_ukm-{{ $ukm->ukm_id }}"
        name="profile_photo_ukm">
      </div>
      <div class="mb-3">
      <label for="name_ukm-{{ $ukm->ukm_id }}" class="form-label">Nama UKM</label>
      <input type="text" class="form-control shadow-none" id="name_ukm-{{ $ukm->ukm_id }}" name="name_ukm"
        value="{{ $ukm->name_ukm }}" required>
      </div>
      <div class="mb-3">
      <label for="description-{{ $ukm->ukm_id }}" class="form-label">Deskripsi UKM</label>
      <textarea class="form-control shadow-none" name="description" id="description-{{ $ukm->ukm_id }}"
        required>{{ $ukm->description }}</textarea>
      </div>
      <div class="mb-3">
  ...
  {{-- delete ukm --}}
  @foreach ($ukms as $ukm)
    <div class="modal fade" id="deleteUkm-{{ $ukm->ukm_id }}" tabindex="-1" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-footer">
      <button type="button" class="btn btn-secondary shadow-none" data-bs-dismiss="modal">Close</button>
      <button type="submit" class="btn btn-danger shadow-none">Hapus UKM</button>
      </div>
      </form>
      </div>
    </div>
